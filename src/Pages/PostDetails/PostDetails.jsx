...
import { ArrowUp, ArrowDown, MessageCircle, Share2, Calendar, User, Tag } from 'lucide-react';
...
const PostDetails = () => {
    ...
    const voteMutation = useMutation({
        mutationFn: async ({ postId, voteType }) => {
            const response = await axiosSecure.post(`/posts/${postId}/vote`, { voteType });
            return response.data;
        },
        onSuccess: () => {
    ...
    const addCommentMutation = useMutation({
        mutationFn: async ({ postId, comment }) => {
            const response = await axiosSecure.post(`/posts/${postId}/comments`, { comment });
            return response.data;
        },
        onSuccess: () => {
                    ...
                    {user ? (
                        ...
                        <div className="mb-8 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                            <p className="text-blue-800 dark:text-blue-200">
                                Please <Link to="/login" className="underline font-medium">login</Link> to add a comment.
                            </p>
                        </div>
                    )}
                    ...
                    {commentsLoading ? (
                        ...
                        <div className="space-y-6">
                            {comments.map((comment) => (
                                <div key={comment._id} className="border-b border-gray-200 dark:border-gray-700 pb-6 last:border-b-0">
                                    <div className="flex items-start space-x-3">
                                        <img
